...
def validate_rural_coordinate(lat, lon, metadata, gmaps, debug=False):
    ...
    try:
        ...
        max_establishments = 25  # Threshold for rural validation
        if len(establishments) > max_establishments:
            if debug:
...
def get_roads_near_coordinate(lat, lon, radius=20000):
    """
    Get road network near a coordinate using Overpass API (OpenStreetMap data).
    This helps us find locations that are likely to have Street View coverage.
    """
    overpass_url = "http://overpass-api.de/api/interpreter"
    
    # Query for roads within radius
    overpass_query = f"""
    [out:json][timeout:10];
    (
            ...
            for element in data.get('elements', []):
                if 'geometry' in element:
                    for point in element['geometry']:
                        roads.append((point['lat'], point['lon']))
            return roads
        return []
    except Exception as e:
...
def pre_scan_streetview_coverage(areas_list, continent, region_type, num_samples=100):
    """
    Pre-scan an area to find coordinates with Street View coverage.
    Cache results to avoid repeated API calls.
    """
    cache_file = f"cache/{continent}_{region_type}_streetview_coverage.pkl"
    ...
    if os.path.exists(cache_file):
        try:
            with open(cache_file, 'rb') as f:
                ...
                if len(cached_coords) > 20:  # If we have enough cached coordinates
                    print(f"  📋 Loaded {len(cached_coords)} cached coordinates for {continent}-{region_type}")
                    return cached_coords
        except Exception as e:
            print(f"Error loading cache: {e}")
    
    print(f"  🔍 Pre-scanning Street View coverage for {continent}-{region_type}...")
    
    valid_coords = []
    
    # Strategy 1: Sample road networks
    for area in areas_list:
        # Sample a few points in the area
        center_lat = (area['min_lat'] + area['max_lat']) / 2
        center_lon = (area['min_lon'] + area['max_lon']) / 2

        roads = get_roads_near_coordinate(center_lat, center_lon, radius=20000)

        if roads:
            # Sample road points and check Street View availability
            road_sample = random.sample(roads, min(20, len(roads)))
            
            for road_lat, road_lon in road_sample:
                # Quick metadata check (without full processing)
                if quick_streetview_check(road_lat, road_lon):
                    valid_coords.append((road_lat, road_lon))
                    
                if len(valid_coords) >= num_samples:
                    break
    
    # Strategy 2: Grid sampling with distance from roads
    if len(valid_coords) < num_samples // 2:
        print(f"  🌐 Grid sampling for additional coordinates...")
        
        for area in areas_list:
            # Create a grid and sample points
            lat_step = (area['max_lat'] - area['min_lat']) / 10
            lon_step = (area['max_lon'] - area['min_lon']) / 10
            
            for lat in [area['min_lat'] + i * lat_step for i in range(1, 10, 2)]:
                for lon in [area['min_lon'] + j * lon_step for j in range(1, 10, 2)]:
                    if quick_streetview_check(lat, lon):
    ...
    try:
        with open(cache_file, 'wb') as f:
            pickle.dump(valid_coords, f)
        print(f"  💾 Cached {len(valid_coords)} valid coordinates")
    except Exception as e:
        print(f"Error saving cache: {e}")
    
    return valid_coords

def quick_streetview_check(lat, lon):
    """
    Quick Street View availability check without full processing.
    """
    url = f"https://maps.googleapis.com/maps/api/streetview/metadata?location={lat},{lon}&key={keys.API_KEY}"
    try:
...
def smart_coordinate_generation(valid_coords_pool, used_coords, min_distance=0.001):
    """
    Generate coordinates from pre-validated pool, avoiding duplicates.
    """
    if not valid_coords_pool:
        return None, None
    
    import random
    
    # Try to find unused coordinates
    for _ in range(50):  # Max 50 attempts
        coord = random.choice(valid_coords_pool)
        lat, lon = coord
        
        # Check if too close to already used coordinates
        too_close = False
        for used_lat, used_lon in used_coords:
            distance = ((lat - used_lat) ** 2 + (lon - used_lon) ** 2) ** 0.5
    ...
    # If we can't find unused coordinates, return a random one anyway
    coord = random.choice(valid_coords_pool)
...
if __name__ == "__main__":
    ...
    for continent, regions in AREAS.items():
        for region_type in regions.keys():
            ...
            overview[f"{continent}-{region_type}"] = {
                "existing": existing,
                "needed": needed
            }
    ...
    # Pre-scan all areas for Street View coverage
    coverage_cache = {}
    
    for continent, regions in AREAS.items():
        coverage_cache[continent] = {}
        for region_type, areas_list in regions.items():
            if overview[f"{continent}-{region_type}"]['needed'] <= 0:
                print(f"  Skipping pre-scan for {continent}-{region_type} - already has {overview[f'{continent}-{region_type}']['existing']} images")
                continue
            print(f"\n🔍 Pre-scanning {continent} - {region_type}...")
            valid_coords = pre_scan_streetview_coverage(areas_list, continent, region_type, num_samples=200)
            coverage_cache[continent][region_type] = {
                'coords': valid_coords,
                'used': set()
            }
            print(f"   Found {len(valid_coords)} coordinates with Street View coverage")
    
    print("\n" + "=" * 60)
    print("Starting smart dataset generation with pre-validated coordinates...")
    print("=" * 60)
    ...
    for continent, regions in AREAS.items():
        ...
        for region_type, areas_list in regions.items():
            ...
            if remaining_needed == 0:
                print(f"    ✓ Already completed {continent}-{region_type}")
                continue
            
            print(f"  {region_type.capitalize()}: Need {remaining_needed} more images")
            
            # Get pre-validated coordinates
            coord_cache = coverage_cache[continent][region_type]
            valid_coords_pool = coord_cache['coords']
            used_coords = coord_cache['used']
            
            if not valid_coords_pool:
                print(f"    ❌ No valid coordinates found for {continent}-{region_type}")
                continue
            
            print(f"    📍 Using pool of {len(valid_coords_pool)} pre-validated coordinates")
            ...
            failed_attempts = 0
            max_failed_attempts = 5000  # Much lower since we're using pre-validated coords
            
            while image_count < args.num_images and failed_attempts < max_failed_attempts:
                # Use smart coordinate generation
                lat, lon = smart_coordinate_generation(valid_coords_pool, used_coords)
                
                if lat is None:
                    print(f"    ⚠ Exhausted coordinate pool for {continent}-{region_type}")
                    break
